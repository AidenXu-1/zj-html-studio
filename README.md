...
- `0.1.3` 已在 macOS / Obsidian 1.12.7 完成真实运行验收
- 自动检查会在 macOS、Windows、Linux 上执行 lint、类型检查、75 项测试、生产构建、发布资产校验和高危依赖审计
...
插件没有运行时第三方依赖。生产安装文件合计约 72 KB；`node_modules/`、`dist/`、本地设置、日志和环境变量文件均不会进入 Git 历史。
